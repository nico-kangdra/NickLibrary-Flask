...
def registerpage():
    ...
    if request.method == "POST":
        ...
        password = hashlib.md5(
            request.form['Rpassword'].encode()).hexdigest()  # hash the password
        name = request.form['Rname']
        passwordcon = hashlib.md5(
            request.form['passwordcon'].encode()).hexdigest()
        ...
        if not check:
            if password == passwordcon:
                ...
                sql = """INSERT INTO users (username, password, name, type) VALUES('%s','%s','%s','%s')""" % (
                    username, password, name, "user" if username.lower() != "admin" else "admin")
                cur.execute(sql)
                conn.commit()
                cur.close()
                conn.close()
